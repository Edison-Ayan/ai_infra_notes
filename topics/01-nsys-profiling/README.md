...
**修好单个瓶颈必要但不充分**——必须同时把寄存器/occupancy/访存指令/ILP 全压平衡。手写每次只动一个变量，几乎必然按下葫芦浮起瓢。**这正是 cuBLAS/CUTLASS 的价值：在整个配置空间自动调参找全局平衡点。** float4(6.13, 88% cuBLAS) 是个难靠单点改动超越的局部最优；要再上得像 CUTLASS 那样 tile/线程数/寄存器一起调。
