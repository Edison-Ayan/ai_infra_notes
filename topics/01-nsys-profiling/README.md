...
| | v0 baseline | pad | swizzle | **warp tiling** |
|---|---|---|---|---|
| TFLOPS | 6.14 | 6.19 | 6.35 | 5.78 |
| **load 冲突** | 5.0-way | 5.0-way ✗ | 5.0-way ✗ | **消失** ✓✓ |
| store 冲突 | 2.4-way | 消失 ✓ | 消失 ✓ | 2.4-way |
| excessive wavefront | 40% | 38% | 38% | **4%** ✓✓ |
| L1/TEX 吞吐 | 83% | 82% | 84% | **45.8%** ✓✓ |
| 寄存器/线程 | 127 | ~127 | ~127 | **232** ✗ |
| Occupancy | 32% | 32% | 32% | **16.7%** ✗ |
| FP32% | 53% | 54% | 55% | 47% |

（warp tiling 版见 [labs/gemm_warptile.cu](labs/gemm_warptile.cu)）
...
**为什么"剩下 4 个 lane 读同列"是广播，不是冲突**：32 lane 排成 8(N)×4(M)，同一列上有 4 个 lane(tColW 同、tRowW 不同)。
读 B 的索引 `Bs[...+tColW*TN+j]` **只含 tColW、不含 tRowW** → 这 4 个 lane 算出同一地址 → 读同址=**广播(免费)**。
根子在 GEMM 的数学：`C[m][n]=Σ A[m][k]·B[k][n]`，**B[k][n] 与行 m 无关** → 同列不同行的线程必然要同一个 B 值。

| 读什么 | 地址只跟谁有关 | 谁广播 |
|---|---|---|
| regN(B) | tColW(列) | 同列的 4 个 tRowW lane 读同址 |
| regM(A) | tRowW(行) | 同行的 8 个 tColW lane 读同址 |

> **GEMM 的复用地基**：B 沿行方向被复用、A 沿列方向被复用(分块加速的根本)。所以"同列不同行"的线程读 B 必撞同址——
> 撞**同址=广播(免费)**，撞**同 bank 不同址=冲突(要命)**。warp tiling 的布局让该广播的广播、该分散的(8×4=32)正好铺满 bank。
...
| 优化 | 治好 | 被反噬 |
|---|---|---|
| db 双缓冲 | 想藏访存延迟 | 寄存器悬崖→occupancy 16.7% |
| cpasync | 寄存器悬崖 ✓ | 转置丢失→regM 标量 |
| pad/swizzle | store 冲突 ✓ | 没碰主瓶颈 load 冲突 |
| warp tiling | **load 冲突 ✓✓** | 寄存器 232→occupancy 16.7% |

**修好单个瓶颈必要但不充分**——必须同时把寄存器/occupancy/访存指令/ILP 全压平衡。手写每次只动一个变量，几乎必然按下葫芦浮起瓢。

### 破局：wt2 调优版——同时摆平所有维度（[labs/gemm_wt2.cu](labs/gemm_wt2.cu)）
同样是 warp tiling，只换 4 个参数（`WMITER=2,WNITER=2,TM=4,TN=4` + 256 线程），把每线程累加器从 128→64：

| | float4 | warptile(128线程) | **wt2(256线程)** |
|---|---|---|---|
| TFLOPS | 6.13 | 5.78 | **6.82 (98% cuBLAS)** |
| load 冲突 | 5-way | 消失 | **消失** ✓ |
| 寄存器/线程 | 127 | 232 | **127** ✓ |
| Occupancy | 32% | 16.7% | **32%** ✓ |
| FP32% | 53% | 47% | **60%** ✓ |

三维第一次同时压平：**无冲突访问(warp tiling) + 寄存器回 float4 预算(累加器减半) + occupancy 回 32%(256线程)**。

> **"多维度拔河"是可解的**——方法不是每次动一个变量，而是把"无冲突访问 + 寄存器预算 + occupancy"当**联立方程一起解**。在配置空间搜最优解正是 **CUTLASS/cuBLAS 的 autotuning**：我们手凑一组参数摸到 98%，它们自动扫几百组找全局最优。这就是那种库的价值与它到底在调什么。
...
| float4 | 2.80 ms | 6.13 | 88% | 53% |
| **wt2 (warp tiling 调优, 本实验最优)** | 2.52 ms | **6.82** | **98%** | 60% |
