// =============================================================
// nsys 学习实验台 (gemm_lab_pinned) —— pinned 内存对比版
//
// 与 gemm_lab.cu 唯一的区别：host 端的 h_A/h_B/h_C 用
// cudaMallocHost (pinned / 锁页内存) 分配，而不是 malloc。
// 目的是对比 H2D/D2H 拷贝带宽：pageable vs pinned。
//
...
int main() {
    ...
    // ---- host 内存 + 初始化 ----
    // pinned (锁页) 内存：driver 不必再走内部 pinned 中转缓冲，
    // DMA 可直接搬运 => H2D/D2H 带宽通常翻倍。
    float *h_A, *h_B, *h_C;
