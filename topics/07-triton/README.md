...
| 文件 | 内容 |
|---|---|
| [labs/triton_gemm.py](labs/triton_gemm.py) | lab① Triton 分块 GEMM + autotune，对线 cuBLAS(torch.matmul)，FP32/FP16 |
| [labs/triton_fused_gelu.py](labs/triton_fused_gelu.py) | lab② matmul+bias+GELU **融合**一个 kernel，对比 torch 三独立 kernel |
| [labs/run.sh](labs/run.sh) | 跑脚本（python 无需编译，故用 run.sh 不是 build.sh） |

```bash
cd labs
./run.sh              # lab① FP32（两边对齐 TF32）
./run.sh fp16         # lab① FP16 走 tensor core
DUMP=1 ./run.sh fp16  # lab① 额外 dump ttir/ttgir/ptx，看 lowering
./run.sh fuse         # lab② 融合对比（FP16，跑三个形状）
...
## 实验③：算子融合 —— matmul + bias + GELU 一个 kernel（新维度）

lab① 优化的是**单 kernel 内部**；融合第一次让我碰 kernel **之间**的访存——我手写 CUDA
笔记里完全没有的维度。机制：

- **不融合(torch 三步)**：matmul 写 M×N 回 HBM → +bias 再读+写 M×N → gelu 再读+写 M×N。
  中间大矩阵在 HBM 来回搬 = 纯访存浪费。
- **融合(Triton 一步)**：matmul 累加器**还在寄存器里**，就地加 bias + GELU，**只写一次** M×N。
  省掉中间结果的 2 读 + 2 写。实现就是在 GEMM 的 store 之前插一段 epilogue（[triton_fused_gelu.py](labs/triton_fused_gelu.py) 第 79–84 行）。

**核心 insight（实测，FP16，中间结果恒为 M×N=4096²）**：

| 形状 | 融合 | 不融合(3 kernel) | 加速 | 省搬 |
|---|---|---|---|---|
| K=4096（算力 bound） | 5.28 ms | 6.20 ms | **1.17×** | 0.13 GB |
| K=1024（中间） | 1.43 ms | 2.00 ms | **1.40×** | 0.13 GB |
| K=256（访存倾斜） | 0.42 ms | 0.96 ms | **2.28×** | 0.13 GB |

> **法则：融合省的访存是固定的(0.13GB)，但 matmul 越不算力 bound，这块省下的占比越大 → 融合越赚。**
> K=4096 时 matmul 算力占满、融合只 1.17×；K=256 时 matmul 很轻、epilogue 访存成主角 → 2.28×。
> 推论：decode/推理那种**瘦 matmul + 一堆 elementwise**，融合是大杠杆——这正是 torch.compile 默认猛融的原因。
...
- **融合 = 优化 kernel 之间的访存**（手写 CUDA 没有的维度）：累加器在寄存器里就地做 epilogue，省中间结果往返 HBM；越访存 bound 越赚（1.17×→2.28×）。

## TODO / 下一步

- [x] 实验①：Triton GEMM 对线 cuBLAS（FP32 110% / FP16 105%）
- [x] 实验②：dump ttgir/ptx，验证编译器自动发 cp.async + mma.sync
- [x] 实验③：融合 `matmul+bias+GELU` 一个 kernel，越访存 bound 越赚（1.17×→2.28×）
